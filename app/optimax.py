...
# (actual inputs are in the Options Trading tab)
# ─────────────────────────────────────────────
...
<style>
    ...
    .stButton > button[kind="primary"] {
        background: linear-gradient(135deg, #6C63FF 0%, #5A4FE0 100%);
        border: none;
    }
    ...
    if not st.session_state.sidebar_chat_messages:
        st.markdown(
            """<div class="chat-welcome">
                <h4>Welcome to OptiMax AI</h4>
                <p>Ask me anything about investing, trading strategies, portfolio ideas, or market analysis.</p>
            </div>""",
            unsafe_allow_html=True,
        )
        # Suggestion chips
        ...
        for msg in st.session_state.sidebar_chat_messages:
            if msg["role"] == "user":
                ...
                st.markdown(f'<div class="chat-label">Advisor</div><div class="chat-msg-advisor">{msg["content"]}</div>', unsafe_allow_html=True)
...
# Check if there's a pending suggestion or new user input
...
if pending and st.session_state.sidebar_chat_messages and st.session_state.sidebar_chat_messages[-1]["role"] == "user":
    ...
    if not groq_key or groq_key == "your-api-key-here":
        ...
        try:
            ...
            context = f"The user is analyzing {symbol}. "
            context += f"Their outlook is {outlook_key}, risk tolerance is {risk_tol_key}, "
            context += f"portfolio value is ${portfolio_value:,}, max risk per trade is {risk_per_trade*100:.1f}%."

            system_prompt = f"""You are an expert AI investment advisor built into OptiMax, an options and portfolio management tool.
You help users discuss investment strategies, stock analysis, options trading, portfolio diversification, and market trends.
Be concise since responses appear in a sidebar. Use bullet points when helpful. Keep responses under 200 words.
Current context: {context}
Always remind users this is for educational/discussion purposes, not financial advice."""
            ...
            st.session_state.sidebar_chat_messages.append({"role": "assistant", "content": assistant_reply})
        except Exception as e:
            st.session_state.sidebar_chat_messages.append({"role": "assistant", "content": f"Error: {str(e)}"})
        st.rerun()
...
# ─────────────────────────────────────────────
# Load Data (for options — loaded lazily)
# ─────────────────────────────────────────────
            ...
            if file_name.endswith(".csv"):
                ...
                try:
                    ...
                    for col in df.columns:
                        ...
                        if col_lower in ("symbol", "ticker", "stock", "sym"):
                            col_map["symbol"] = col
                        elif col_lower in ("shares", "quantity", "qty", "units", "amount"):
                            col_map["shares"] = col
                        elif col_lower in ("cost", "price", "avg cost", "avg_cost", "cost basis", "cost_basis", "purchase price", "buy price"):
                            col_map["cost"] = col
                        elif col_lower in ("date", "purchase date", "purchase_date", "buy date", "buy_date"):
                            col_map["date"] = col
                        elif col_lower in ("notes", "note", "comment", "comments"):
                            col_map["notes"] = col

                    if "symbol" in col_map and "shares" in col_map:
                        st.success(f"Detected {len(df)} rows — Symbol: '{col_map['symbol']}', Shares: '{col_map['shares']}'")
                        parsed = []
                        for _, row in df.iterrows():
                            ...
                            date_val = None
                            if "date" in col_map and pd.notna(row.get(col_map["date"])):
                                try:
                                    date_val = pd.to_datetime(row[col_map["date"]])
                                except Exception:
                        ...
                        st.warning(
                            "Could not auto-detect columns. Please make sure your Excel file has "
                            "columns named **Symbol** (or Ticker) and **Shares** (or Quantity)."
                        )
                except ImportError:
    ...
    with st.expander("**Settings** — Ticker, Market View & Position Sizing", expanded=True):
        ...
        with opt_col2:
            ...
            outlook_map = {
                ...
                "Neutral": "neutral", "Volatile (big move, unsure direction)": "volatile",
            }
        ...
        with opt_col3:
            ...
            portfolio_value = st.number_input(
                "Portfolio Value ($)",
                min_value=1000, max_value=10_000_000, value=25_000, step=1000,
                help="Your total trading portfolio size",
                key="opt_portfolio_val",
            )
